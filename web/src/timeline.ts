...
import {
  axisH, badgeH, lPad, rPad, rowH, shapeCornerR, shapeStrokeW, timelineFontSize,
  maxTimelineTitleW, palette, hhmm, timeMap, hourGridlines,
} from './layout'
...
export function renderTimeline(ctx: DrawCtx, p: TimelineProps): void {
  ...
  for (const cur of hourGridlines(rs, re)) {
    const x = t2x(cur)
    ctx.stroke(ctx.beginPath().moveTo(x, axisH).lineTo(x, bottomY), palette.separator, 0.5)

    const lbl = String(cur.getHours())
    const sz = ctx.measureText(lbl, lblFont)
    ctx.fillText(lbl, x - sz.width / 2, bottomY + (axisH - sz.height) / 2, lblFont, palette.secondaryLabel)
  }
  ...
  if (now >= rs && now <= re) {
    const nx = t2x(now)
    const dotR = 3
    ctx.stroke(ctx.beginPath().moveTo(nx, axisH / 2).lineTo(nx, bottomY), palette.systemRed, 1, 0.5)
    ctx.fill(ctx.beginPath()
      .arc(nx, axisH / 2, dotR, 0, 360, true), palette.systemRed)

    const nowFont = font(timelineFontSize - 2, 'semibold', true)
    const nowStr = hhmm(now)
    const nsz = ctx.measureText(nowStr, nowFont)
    ctx.fillText(nowStr, nx + dotR + 3, (axisH - nsz.height) / 2, nowFont, palette.systemRed)
  }
  ...
  events.forEach((ev, i) => {
    ...
    const isFocused = p.focused != null && p.focused.start.getTime() === ev.start.getTime() && p.focused.title === ev.title
    ...
    if (isAfternoon) {
      const maxW = Math.min(maxTimelineTitleW, Math.max(0, x1 - 6 - lPad))
      const drawW = Math.min(natural.width, maxW)
      titleRect = { x: x1 - 6 - drawW, y: cy - titleH / 2, w: drawW, h: titleH }
    } else {
      const maxW = Math.max(0, w - rPad - afterTimeX)
      const drawW = Math.min(natural.width, maxW)
      titleRect = { x: afterTimeX, y: cy - titleH / 2, w: drawW, h: titleH }
    }

    layouts.push({
      cy, x1, x2, color, alpha, timeStr, timeFont, timeOrigin, timeSz,
      titleStr: ev.title, titleFont, titleColor, titleRect,
    })
  })

  // Pass 1: white backgrounds behind text (sized to rendered width, inset -2).
  for (const l of layouts) {
    ctx.fillRect(l.timeOrigin.x - 2, l.timeOrigin.y, l.timeSz.width + 4, l.timeSz.height, palette.white)
    ctx.fillRect(l.titleRect.x - 2, l.titleRect.y, l.titleRect.w + 4, l.titleRect.h, palette.white)
  }
  ...
  // Pass 3: text on top.
  for (const l of layouts) {
    ctx.fillText(l.timeStr, l.timeOrigin.x, l.timeOrigin.y, l.timeFont, l.color, l.alpha)
